...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-shipping-service', url: 'https://github.com/NwamakaAgbakoba/realworld-microservice-project.git'
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker nwamakaagbakoba/shippingservice:latest || true" 
            }
        }
